...
$result = pg_prepare($connection, "auth_query", 'SELECT * FROM plusones WHERE user_id = $1 AND entry_id = $2');
...
if ( $row[0] > 0 ) { 
  ...
  $result = pg_prepare($connection, "delete_query", "DELETE FROM plusones WHERE entry_id = $1 AND user_id = $2");
  $result = pg_execute($connection, "delete_query", array( $_GET['entry_id'] , $_SESSION['user_id']) );    
  
  $result = pg_prepare($connection, "update_query", "UPDATE blog SET score = score - 1 WHERE ID = $1");
  $result = pg_execute($connection, "update_query", array( $_GET['entry_id'] ) );
} else {
  // User has not liked this entry, so add it to the score
  $result = pg_prepare($connection, "insert_query", "INSERT INTO plusones (user_id, entry_id) VALUES ($1, $2)");
